...
an RPM building environment configured with rpmdev-setuptree. Then, for each release, do the following:

To create RPMs from an existing github release
---------------------------------------------

0. For what follows, use the following definitions. You can even set shell variables accordingly so you can just copy paste commands.
...
5. Create a patch file containing all the changes since the specified commit 
by running:

    git diff $GITHUB_RELEASE_NUMBER \
	> ~/rpmbuild/SOURCES/${PACKAGE_NAME}-rpm.patch

6. Download the appropriate sources into ~/rpmbuild/SOURCES with:
...
If there are problems, fix them and return to step 4.

8. Commit and push the updated spec file to the official github repository.
